...
#import "JpMasuidriveTiEvernoteapiEDAMAdImpressionsProxy.h"
...
{
    return [[[JpMasuidriveTiEvernoteapiEDAMAdParametersProxy alloc] initWithObject: [[[EDAMAdParameters alloc] init] autorelease]] autorelease];
}
@end


@implementation JpMasuidriveTiEvernoteapiEDAMAdParametersProxy
@synthesize object;
...
-(id)impressions
{
    return arrayMap(object.impressions, ^(id item) { return [[[JpMasuidriveTiEvernoteapiEDAMAdImpressionsProxy alloc] initWithObject: item] autorelease]; });
}

-(void)setImpressions:(id)value
{
    object.impressions = arrayMap(value, ^(id item) { return ((JpMasuidriveTiEvernoteapiEDAMAdImpressionsProxy*)item).object; });
}
